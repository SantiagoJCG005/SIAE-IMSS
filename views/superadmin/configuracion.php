...
// Funcion auxiliar para cargar configuracion por tipo
function cargarConfiguracion($conexion, $tipo) {
    $config = [];
    try {
...
// Carga las configuraciones segun la pestana activa
$configGeneral = [];
$configAcademico = [];
$configSeguridad = [];
$configSmtp = [];
...
if ($pestana === 'general') {
    $configGeneral = cargarConfiguracion($conexion, 'general');
} elseif ($pestana === 'academico') {
    $configAcademico = cargarConfiguracion($conexion, 'academico');
} elseif ($pestana === 'seguridad') {
    $configSeguridad = cargarConfiguracion($conexion, 'seguridad');
} elseif ($pestana === 'smtp') {
    $configSmtp = cargarConfiguracion($conexion, 'smtp');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postTab === 'imss') {
        ...
        $datos = [
            'registro_patronal' => obtenerPost('registro_patronal'),
            'digito_verificador' => obtenerPost('digito_verificador'),
            'umf_alta' => obtenerPost('umf_alta'),
            'umf_baja' => obtenerPost('umf_baja'),
            'codigo_operacion_alta' => obtenerPost('codigo_operacion_alta'),
            'codigo_operacion_baja' => obtenerPost('codigo_operacion_baja'),
            'prefijo_alta' => obtenerPost('prefijo_alta'),
            'prefijo_baja' => obtenerPost('prefijo_baja'),
            'codigo_institucion' => obtenerPost('codigo_institucion')
        ];
...
<div class="card">
    ...
    <div class="card-body">
        <form id="formGeneral" onsubmit="guardarConfiguracion(event, 'general')">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nombre_institucion" class="form-control" 
                       value="<?= htmlspecialchars($configGeneral['nombre_institucion'] ?? '') ?>" 
                       placeholder="Nombre de tu institución" maxlength="100" required>
            </div>
            <div class="form-group">
                <label class="form-label">Zona Horaria</label>
                <select name="zona_horaria" class="form-control form-select">
                    <option value="America/Mexico_City" <?= ($configGeneral['zona_horaria'] ?? '') === 'America/Mexico_City' ? 'selected' : '' ?>>Ciudad de México (UTC-6)</option>
                    <option value="America/Tijuana" <?= ($configGeneral['zona_horaria'] ?? '') === 'America/Tijuana' ? 'selected' : '' ?>>Tijuana (UTC-8)</option>
                    <option value="America/Cancun" <?= ($configGeneral['zona_horaria'] ?? '') === 'America/Cancun' ? 'selected' : '' ?>>Cancún (UTC-5)</option>
                </select>
            </div>
        </form>
    </div>
    <div class="card-footer">
        <button type="submit" form="formGeneral" class="btn btn-primary">
...
<div class="card">
    ...
    <div class="card-body">
        <form id="formAcademico" onsubmit="guardarConfiguracion(event, 'academico')">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                <div class="form-group">
                    <label class="form-label">Periodo Escolar Activo</label>
                    <select name="periodo_activo" class="form-control form-select">
                        <?php 
                        $periodos = $conexion->query("SELECT * FROM periodo_escolar ORDER BY fecha_inicio DESC")->fetchAll();
                        foreach ($periodos as $periodo): ?>
                        <option value="<?= $periodo['id_periodo'] ?>" <?= ($configAcademico['periodo_activo'] ?? '') == $periodo['id_periodo'] ? 'selected' : '' ?>><?= htmlspecialchars($periodo['nombre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Formato Número de Control</label>
                    <input type="text" name="formato_control" class="form-control" 
                           value="<?= htmlspecialchars($configAcademico['formato_control'] ?? 'AAXXNNNN') ?>" 
                           placeholder="Ej: 25010001" maxlength="20">
            ...
            <div class="form-group">
                ...
                <input type="number" name="dias_gracia" class="form-control" 
                       value="<?= htmlspecialchars($configAcademico['dias_gracia'] ?? '5') ?>" 
    ...
    <div class="card-footer">
        <button type="submit" form="formAcademico" class="btn btn-primary">
            <i data-lucide="save"></i>
            Guardar cambios
        </button>
    </div>
...
<div class="card">
    ...
    <div class="card-body">
        <form id="formSeguridad" onsubmit="guardarConfiguracion(event, 'seguridad')">
            <div class="form-group">
                <label class="form-label">Duración de sesión (minutos)</label>
                <input type="number" name="duracion_sesion" class="form-control" 
                       value="<?= htmlspecialchars($configSeguridad['duracion_sesion'] ?? '60') ?>" 
                       min="15" max="480" style="max-width: 150px;">
            </div>
            <div class="form-group">
                ...
                <input type="number" name="intentos_login" class="form-control" 
                       value="<?= htmlspecialchars($configSeguridad['intentos_login'] ?? '5') ?>" 
                       min="3" max="10" style="max-width: 150px;">
            </div>
            <div class="form-group">
                <label class="form-label">Tiempo de bloqueo (minutos)</label>
                <input type="number" name="tiempo_bloqueo" class="form-control" 
                       value="<?= htmlspecialchars($configSeguridad['tiempo_bloqueo'] ?? '30') ?>" 
                       min="5" max="120" style="max-width: 150px;">
            </div>
            <div class="form-group">
                <label class="form-label" style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="checkbox" name="forzar_cambio_password" value="1" 
                           <?= ($configSeguridad['forzar_cambio_password'] ?? '1') == '1' ? 'checked' : '' ?> style="cursor: pointer;">
                    Forzar cambio de contraseña cada 90 días
                </label>
            </div>
            <div class="form-group">
                <label class="form-label" style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="checkbox" name="password_fuerte" value="1" 
                           <?= ($configSeguridad['password_fuerte'] ?? '1') == '1' ? 'checked' : '' ?> style="cursor: pointer;">
    ...
    <div class="card-footer">
        <button type="submit" form="formSeguridad" class="btn btn-primary">
            <i data-lucide="save"></i>
            Guardar cambios
        </button>
    </div>
...
<div class="card">
    ...
    <div class="card-body">
        <form id="formSmtp" onsubmit="guardarConfiguracion(event, 'smtp')">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
                <div class="form-group">
                    <label class="form-label">Servidor SMTP</label>
                    <input type="text" name="smtp_servidor" class="form-control" 
                           value="<?= htmlspecialchars($configSmtp['smtp_servidor'] ?? '') ?>" 
                           placeholder="smtp.gmail.com" maxlength="100">
                </div>
                <div class="form-group">
                    <label class="form-label">Puerto</label>
                    <input type="number" name="smtp_puerto" class="form-control" 
                           value="<?= htmlspecialchars($configSmtp['smtp_puerto'] ?? '587') ?>" 
                           min="1" max="65535">
                </div>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                <div class="form-group">
                    ...
                    <input type="text" name="smtp_usuario" class="form-control" 
                           value="<?= htmlspecialchars($configSmtp['smtp_usuario'] ?? '') ?>" 
                           placeholder="correo@institucion.edu.mx" maxlength="100">
                </div>
                <div class="form-group">
                    <label class="form-label">Contraseña</label>
                    <input type="password" name="smtp_password" class="form-control" 
                           value="<?= htmlspecialchars($configSmtp['smtp_password'] ?? '') ?>" 
                           placeholder="••••••••" maxlength="100">
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">Encriptación</label>
                <select name="smtp_encriptacion" class="form-control form-select" style="max-width: 200px;">
                    <option value="tls" <?= ($configSmtp['smtp_encriptacion'] ?? '') === 'tls' ? 'selected' : '' ?>>TLS</option>
                    <option value="ssl" <?= ($configSmtp['smtp_encriptacion'] ?? '') === 'ssl' ? 'selected' : '' ?>>SSL</option>
                    <option value="none" <?= ($configSmtp['smtp_encriptacion'] ?? '') === 'none' ? 'selected' : '' ?>>Ninguna</option>
                </select>
            </div>
            <div class="form-group">
                <label class="form-label">Email remitente</label>
                <input type="email" name="smtp_email_remitente" class="form-control" 
                       value="<?= htmlspecialchars($configSmtp['smtp_email_remitente'] ?? '') ?>" 
                       placeholder="noreply@institucion.edu.mx" maxlength="100">
            </div>
            <div class="form-group">
                <label class="form-label">Nombre remitente</label>
                <input type="text" name="smtp_nombre_remitente" class="form-control" 
                       value="<?= htmlspecialchars($configSmtp['smtp_nombre_remitente'] ?? '') ?>" 
                       placeholder="SIAE-IMSS Sistema" maxlength="100">
            </div>
        </form>
    </div>
    <div class="card-footer" style="display: flex; gap: 12px;">
        <button type="button" class="btn btn-outline" onclick="probarCorreo()">
            <i data-lucide="send"></i>
            Enviar correo de prueba
        </button>
        <button type="submit" form="formSmtp" class="btn btn-primary">
            <i data-lucide="save"></i>
            Guardar cambios
        </button>
    </div>
...
<script>
// Funcion para guardar la configuracion de cualquier pestaña
async function guardarConfiguracion(evento, tipo) {
    ...
    // Para checkboxes, verifica si estan marcados
    for (let [clave, valor] of formData.entries()) {
        datos[clave] = valor;
    }
    
    // Agrega los checkboxes no marcados como '0'
    formulario.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        if (!cb.checked) {
            datos[cb.name] = '0';
        }
    });
    
    // Debug: muestra los datos que se van a enviar
    console.log('Guardando configuracion tipo:', tipo);
    console.log('Datos a enviar:', datos);
    
    try {
        // Envia los datos al servidor
        const respuesta = await llamarApi('<?= URL_BASE ?>api/configuracion.php', {
            method: 'POST',
            body: JSON.stringify({
                action: 'guardar',
                tipo: tipo,
        ...
        if (respuesta && respuesta.success) {
            ...
            // Recarga la pagina despues de 1 segundo para mostrar los cambios
...
// Funcion que envia un correo de prueba REAL
function probarCorreo() {
    Swal.fire({
        ...
        preConfirm: (email) => {
            return fetch('<?= URL_BASE ?>api/mail.php?action=probar_smtp', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email_prueba: email })
            })
            .then(r => r.json())
            .then(data => {
        ...
        if (result.isConfirmed) {
            Swal.fire('✅ Enviado', 'Revisa tu bandeja de entrada y spam', 'success');
        }
    });
}
</script>
