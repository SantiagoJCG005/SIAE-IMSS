<?php
/**
 * Vista compartida: Explorador de Carpetas
...
$urlCss         = URL_RECURSOS . "css/$rolVista/carpetas.css";
$urlTabla       = URL_BASE . "views/$rolVista/tabla.php";
$mostrarCreador = ($rolVista === 'jefa');
    ...
    <div class="modal" style="max-width: 500px;">
        ...
        <form id="formTabla" onsubmit="guardarTabla(event)">
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 16px; margin-top: 8px;">
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            ...
                            <span style="display: flex; align-items: center; gap: 4px;"><span style="color: #22C55E;">▲</span> Alta</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            <input type="radio" name="tipo" value="baja">
                            <span style="display: flex; align-items: center; gap: 4px;"><span style="color: #EF4444;">▼</span> Baja</span>
                        </label>
                    </div>
                </div>
                <div class="form-group">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary"><i data-lucide="plus"></i> Crear Tabla</button>
            </div>
        </form>
    </div>
</div>
...
const URL_TABLA    = '<?= $urlTabla ?>';
const MOSTRAR_CREADOR = <?= $mostrarCreador ? 'true' : 'false' ?>;
...
async function cargarCarpetas() {
    ...
    if (carpetas.length === 0) {
        contenedor.innerHTML = `
            <div class="empty-carpetas">
                ...
                <button class="btn btn-primary" onclick="abrirModalCarpeta()">
                    <i data-lucide="plus"></i> Crear Carpeta
                </button>
            </div>`;
        lucide.createIcons();
        return;
    ...
    let html = '';
    carpetas.forEach(carpeta => { html += generarHtmlCarpeta(carpeta); });
    contenedor.innerHTML = html;
    lucide.createIcons();
}

function generarHtmlCarpeta(carpeta) {
    const creadorHtml = MOSTRAR_CREADOR && carpeta.creador
        ? `<br>👤 Por: <strong>${escapeHtml(carpeta.creador.split(' ').slice(0, 2).join(' '))}</strong> <span style="font-size: 11px; color: var(--text-muted);">(${escapeHtml(carpeta.creador_rol || '')})</span>`
        : '';
    let html = `
        <div class="carpeta-item" data-id="${carpeta.id_carpeta}">
            <div class="carpeta-header" onclick="toggleCarpeta(${carpeta.id_carpeta})">
                <div class="carpeta-info">
                    <div class="carpeta-icon"><i data-lucide="folder"></i></div>
                    <div>
                        <div class="carpeta-nombre">${escapeHtml(carpeta.nombre)}</div>
                        <div class="carpeta-meta">
                            ${carpeta.total_subcarpetas} subcarpeta(s) • Creado: ${formatearFecha(carpeta.fecha_creacion)}${creadorHtml}
                        </div>
                    </div>
                </div>
    ...
    if (carpeta.subcarpetas && carpeta.subcarpetas.length > 0) {
        carpeta.subcarpetas.forEach(sub => { html += generarHtmlSubcarpeta(sub, carpeta.nombre); });
    }
    html += `
                <div class="agregar-subcarpeta">
                    <button class="btn btn-ghost btn-sm" onclick="abrirModalSubcarpeta(${carpeta.id_carpeta}, '${escapeHtml(carpeta.nombre)}')">
                        <i data-lucide="folder-plus"></i> Nueva Subcarpeta
                    </button>
                </div>
            </div>
        </div>`;
...
function generarHtmlSubcarpeta(subcarpeta, carpetaNombre) {
    ...
    if (subcarpeta.tablas && subcarpeta.tablas.length > 0) {
        subcarpeta.tablas.forEach(tabla => { html += generarHtmlTabla(tabla); });
    }
    html += '</div>';
    return html;
}
...
function generarHtmlTabla(tabla) {
    ...
    const estadoClase = 'estado-' + tabla.estado;
    const estadoTexto = tabla.estado === 'borrador' ? '📝 Borrador'
                      : tabla.estado === 'validado'  ? '🟢 Validado' : '🚀 Enviado';
    let acciones = `<a href="${URL_TABLA}?id=${tabla.id_tabla}" class="btn btn-ghost btn-sm btn-icon" title="Ver"><i data-lucide="eye"></i></a>`;
...
function toggleCarpeta(idCarpeta) {
    document.getElementById('carpeta-contenido-' + idCarpeta).classList.toggle('activo');
}
...
function editarCarpeta(id, nombre, descripcion) { abrirModalCarpeta(id, nombre, descripcion); }
...
async function guardarCarpeta(evento) {
    ...
    const respuesta = await llamarApi(API_CARPETAS, {
        method: 'POST',
        body: JSON.stringify({ action: id ? 'editar_carpeta' : 'crear_carpeta', id_carpeta: id, nombre, descripcion })
    });
    if (respuesta && respuesta.success) {
...
async function eliminarCarpeta(id, nombre) {
    ...
    if (resultado.isConfirmed) {
        const respuesta = await llamarApi(API_CARPETAS, {
            method: 'POST', body: JSON.stringify({ action: 'eliminar_carpeta', id_carpeta: id })
        });
        if (respuesta && respuesta.success) { mostrarNotificacion(respuesta.message); cargarCarpetas(); }
        else { mostrarNotificacion(respuesta?.message || 'Error al eliminar', 'error'); }
    }
}
...
async function guardarSubcarpeta(evento) {
    ...
    const respuesta = await llamarApi(API_CARPETAS, {
        method: 'POST',
        body: JSON.stringify({ action: id ? 'editar_subcarpeta' : 'crear_subcarpeta', id_subcarpeta: id, id_carpeta: idCarpeta, nombre, descripcion })
    });
    if (respuesta && respuesta.success) {
        mostrarNotificacion(respuesta.message);
...
async function eliminarSubcarpeta(id, nombre) {
    ...
    if (resultado.isConfirmed) {
        const respuesta = await llamarApi(API_CARPETAS, {
            method: 'POST', body: JSON.stringify({ action: 'eliminar_subcarpeta', id_subcarpeta: id })
        });
        if (respuesta && respuesta.success) { mostrarNotificacion(respuesta.message); cargarCarpetas(); }
        else { mostrarNotificacion(respuesta?.message || 'Error al eliminar', 'error'); }
    }
...
async function guardarTabla(evento) {
    ...
    const respuesta = await llamarApi(API_TABLAS, {
        method: 'POST', body: JSON.stringify({ action: 'crear_tabla', ...datos })
    });
    if (respuesta && respuesta.success) {
        mostrarNotificacion(respuesta.message);
        cerrarModalTabla();
        window.location.href = URL_TABLA + '?id=' + respuesta.data.id_tabla;
    } else {
...
async function eliminarTabla(id, nombre) {
    ...
    if (resultado.isConfirmed) {
        const respuesta = await llamarApi(API_TABLAS, {
            method: 'POST', body: JSON.stringify({ action: 'eliminar_tabla', id_tabla: id })
        });
        if (respuesta && respuesta.success) { mostrarNotificacion(respuesta.message); cargarCarpetas(); }
        else { mostrarNotificacion(respuesta?.message || 'Error al eliminar', 'error'); }
    }
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { cerrarModalCarpeta(); cerrarModalSubcarpeta(); cerrarModalTabla(); }
});
