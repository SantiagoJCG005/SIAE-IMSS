<?php
/**
 * Vista compartida: Tabla de Movimientos IMSS
 * Requiere que $rolVista esté definido antes de incluir ('jefa' | 'admin-se').
 */
$sidebarFile          = __DIR__ . "/../layouts/sidebar-$rolVista.php";
$urlCss               = URL_RECURSOS . "css/$rolVista/tabla.css";
$urlCarpetas          = URL_BASE . "views/$rolVista/carpetas.php";
$urlResultados        = URL_BASE . "views/$rolVista/resultados.php";
$tieneFlujoValidacion = ($rolVista === 'admin-se');

$idTabla = intval(obtenerGet('id', 0));

if ($idTabla <= 0) {
    header('Location: ' . $urlCarpetas);
    exit;
}
...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;">
    <div>
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
            <a href="<?= $urlCarpetas ?>" class="btn btn-ghost btn-sm">
...
<div class="acciones-tabla">
    ...
    <div style="display: flex; gap: 12px;">
        <a href="<?= $urlResultados ?>?id=<?= $idTabla ?>" class="btn btn-outline">
            <i data-lucide="file-text"></i> Ver Resultados
        </a>
        <?php if ($rolVista === 'jefa'): ?>
            <?php if ($esEnviado): ?>
...
<div class="modal-overlay" id="modalAgregar">
    <div class="modal" style="max-width: 600px;">
        ...
        <div id="seccionEscaneo" class="escaner-container" style="margin: 0 20px 0 20px; margin-top: 20px;">
            ...
            <small class="escaner-hint">Escanea el QR y todos los campos se llenarán automáticamente</small>
        </div>
...
const URL_CARPETAS = '<?= $urlCarpetas ?>';
...
function parsearEscaneoNSS(textoEscaneado) {
    const resultado = { nss: '', dvAlumno: '', apellidoPaterno: '', apellidoMaterno: '', nombres: '', curp: '', error: null };
    try {
        let texto = textoEscaneado.trim();
        const regexNSS = /social[:\sÑ]+(\d{11})/i;
        const matchNSS = texto.match(regexNSS);
        if (matchNSS) {
            resultado.nss = matchNSS[1].substring(0, 10);
            resultado.dvAlumno = matchNSS[1].substring(10, 11);
        }
        const regexCURP = /CURP[:\sÑ]+([A-Z0-9]{18})/i;
        const matchCURP = texto.match(regexCURP);
        if (matchCURP) resultado.curp = matchCURP[1].toUpperCase();
        ...
        if (matchNombre) {
            ...
            if (partes.length >= 3 && resultado.curp) {
                ...
                if (penultimaPalabra[0] === letraApPat && primerasPalabras[0] === letraNom) {
                    resultado.nombres = primerasPalabras; resultado.apellidoPaterno = penultimaPalabra; resultado.apellidoMaterno = ultimaPalabra;
                } else if (partes[0][0] === letraApPat) {
                    resultado.apellidoPaterno = partes[0]; resultado.apellidoMaterno = partes[1]; resultado.nombres = partes.slice(2).join(' ');
                } else {
                    resultado.nombres = primerasPalabras; resultado.apellidoPaterno = penultimaPalabra; resultado.apellidoMaterno = ultimaPalabra;
                }
                ...
                resultado.nombres = partes[0]; resultado.apellidoPaterno = partes[1];
            }
        }
    } catch (e) { resultado.error = 'Error: ' + e.message; }
    return resultado;
}

function procesarEscaneoNSS(textoEscaneado) {
    const datos = parsearEscaneoNSS(textoEscaneado);
    if (datos.error) { mostrarNotificacion(datos.error, 'error'); return; }
    if (!datos.nss) { mostrarNotificacion('🔴 No se pudo extraer el NSS. Intenta de nuevo.', 'error'); return; }
    const mapaCampos = {
        'agregarNss': datos.nss, 'agregarDv': datos.dvAlumno,
        'agregarApPaterno': datos.apellidoPaterno, 'agregarApMaterno': datos.apellidoMaterno,
        'agregarNombres': datos.nombres, 'agregarCurp': datos.curp
    };
    let camposLlenados = 0;
    for (const [id, valor] of Object.entries(mapaCampos)) {
        const el = document.getElementById(id);
        if (el && valor) {
            el.value = valor; camposLlenados++;
            el.style.backgroundColor = '#D1FAE5'; el.style.borderColor = '#22C55E';
            setTimeout(() => { el.style.backgroundColor = ''; el.style.borderColor = ''; }, 1500);
        }
    }
    if (camposLlenados > 0) mostrarNotificacion(`🟢 ${camposLlenados} Campos llenados. Verifica los datos.`, 'success');
    else mostrarNotificacion('🔴 No se pudieron extraer datos.', 'error');
}

function procesarBuffer() {
    if (bufferEscaneo.length > 20) { procesarEscaneoNSS(bufferEscaneo); document.getElementById('campoEscaneo').value = ''; }
    bufferEscaneo = '';
}
...
function inicializarEscanerNSS() {
    ...
    campoEscaneo.addEventListener('input', function(e) {
        ...
        if (bufferEscaneo.length > 10 && !detectandoEscaneo) detectandoEscaneo = true;
        if (bufferEscaneo.endsWith('||') && bufferEscaneo.length > 30) {
            document.activeElement.blur(); procesarBuffer(); e.target.value = ''; detectandoEscaneo = false; return;
        }
        timeoutEscaneo = setTimeout(() => {
            bufferEscaneo = e.target.value.replace(/[\r\n]+/g, '');
            if (bufferEscaneo.length > 20) {
                document.activeElement.blur(); procesarBuffer(); e.target.value = ''; detectandoEscaneo = false;
            }
        }, 300);
    });
    campoEscaneo.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            ...
            bufferEscaneo = e.target.value.replace(/[\r\n]+/g, '');
            if (bufferEscaneo.length > 15) { document.activeElement.blur(); procesarBuffer(); e.target.value = ''; }
            return false;
        }
    }, true);
    ...
    campoEscaneo.addEventListener('paste', function(e) {
        setTimeout(() => {
            bufferEscaneo = e.target.value.replace(/[\r\n]+/g, '');
            if (bufferEscaneo.length > 15) { procesarBuffer(); e.target.value = ''; }
        }, 100);
    });
}
...
async function cargarAlumnos() {
    ...
    if (!resp || !resp.success) {
        document.getElementById('cuerpoTabla').innerHTML = `<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--danger);">Error al cargar</td></tr>`;
        return;
    }
...
function filtrarAlumnos() { paginaActual = 1; aplicarFiltros(); }
...
async function guardarNuevo(e) { e.preventDefault(); e.stopPropagation(); await enviarNuevoAlumno(false); return false; }
async function guardarYOtro() { await enviarNuevoAlumno(true); }

async function enviarNuevoAlumno(continuar) {
    const datos = {
        action: 'agregar_alumno', id_tabla: ID_TABLA,
    ...
    if (resp && resp.success) {
        mostrarNotificacion(resp.message);
        if (continuar) { document.getElementById('formAgregar').reset(); document.getElementById('campoEscaneo').value = ''; document.getElementById('campoEscaneo').focus(); }
        else cerrarModal('modalAgregar');
        cargarAlumnos();
    } else {
...
async function eliminar(id) {
    const result = await Swal.fire({
        title: '¿Eliminar alumno?', text: 'Esta acción no se puede deshacer.',
    ...
    if (resp && resp.success) { mostrarNotificacion(resp.message); cargarAlumnos(); }
    else mostrarNotificacion(resp?.message || 'Error al eliminar', 'error');
}

<?php if ($tieneFlujoValidacion): ?>
async function solicitarRevision() {
    if (alumnosData.length === 0) { mostrarNotificacion('La tabla no tiene alumnos registrados', 'warning'); return; }
    const result = await Swal.fire({
        title: '¿Solicitar revisión a la Jefa?',
        html: `<p>Se notificará a la Jefa de Servicios que la tabla está lista para revisión.</p>
               <p style="color:var(--text-muted);margin-top:10px;font-size:13px;">Podrá aprobarla o rechazarla desde su panel de Validar.</p>`,
        icon: 'question', showCancelButton: true, confirmButtonText: 'Sí, solicitar',
    ...
    if (resp?.success) mostrarNotificacion(resp.message || 'Solicitud enviada a la Jefa de Servicios', 'success');
    else mostrarNotificacion(resp?.message || 'Error al enviar solicitud', 'error');
}
<?php endif; ?>
...
async function generarTxt() {
    const result = await Swal.fire({
        ...
        html: `<p>Se generará el archivo TXT con <strong>${alumnosData.length}</strong> alumno(s).</p><p style="color:var(--warning);margin-top:12px;">⚠️ Tomará unos segundos para enviar correos de notificación.</p>`,
        icon: 'question', showCancelButton: true, confirmButtonText: 'Sí, generar',
        cancelButtonText: 'Cancelar', confirmButtonColor: '#2563EB'
    });
    if (result.isConfirmed) {
        Swal.fire({ title: 'Generando archivo...', text: 'Esto puede tomar unos segundos (notificando a la Jefa)', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } });
        try {
            const response = await fetch(`${API_URL.replace('tablas.php', 'exportar-txt.php')}?action=generar&id_tabla=${ID_TABLA}`);
            ...
            document.body.appendChild(a); a.click();
            window.URL.revokeObjectURL(url); a.remove();
            Swal.fire({ title: '¡Exportación exitosa!', text: 'El archivo TXT ha sido generado y descargado.', icon: 'success' }).then(() => { window.location.reload(); });
        } catch (error) {
...
async function eliminarTablaCompleta() {
    const result = await Swal.fire({
        ...
        icon: 'warning', showCancelButton: true, confirmButtonText: 'Sí, eliminar todo',
        cancelButtonText: 'Cancelar', confirmButtonColor: '#EF4444'
    });
    if (result.isConfirmed) {
        const resp = await llamarApi(API_URL, { method: 'POST', body: JSON.stringify({ action: 'eliminar_tabla', id_tabla: ID_TABLA }) });
        if (resp && resp.success) { window.location.href = URL_CARPETAS; }
        else mostrarNotificacion(resp?.message || 'Error al eliminar', 'error');
    }
}
...
document.addEventListener('keydown', function(e) {
    ...
    if (modalAgregar && modalAgregar.classList.contains('active')) {
        if (e.key === 'Tab') { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); return false; }
...
document.addEventListener('keypress', function(e) {
    const modalAgregar = document.getElementById('modalAgregar');
    if (modalAgregar && modalAgregar.classList.contains('active')) {
        if (e.key === 'Tab' || e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); return false; }
    }
}, true);
